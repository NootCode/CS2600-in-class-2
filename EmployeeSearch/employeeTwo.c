...
PtrToEmployee searchEmployeeByPhoneNumber(PtrToConstEmployee ptr, int size, char* number){
    return searchEmployeeTable(ptr, size, number, compareEmployeePhoneNumber); 
}

PtrToEmployee searchEmployeeBySalary(PtrToConstEmployee ptr, int size, double targetVal){
    return searchEmployeeTable(ptr, size, &targetVal, compareEmployeeSalary);
}
